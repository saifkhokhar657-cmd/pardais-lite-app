...
function App() {
  ...
  const [fullPage, setFullPage] = useState<'findFriends' | 'level' | 'userProfile' | null>(null);
  ...
  useEffect(() => {
    ...
    const startAuth = async () => {
      ...
      try {
        ...
        if (!mounted) return;
        if (user) {
          // Firebase persistence restores the existing session before the backend profile
          // request finishes. Enter the app immediately; only fall back to onboarding if
          // the backend explicitly says this account is incomplete.
          try {
          ...
          void api.get('/api/auth/me').then(r => {
            const complete = r.data?.onboardingComplete !== false;
            setOnboarding(!complete);
            setAuthenticated(complete);
          }).catch(() => {
            // Keep the restored Firebase session usable if the profile API is temporarily slow.
            setOnboarding(false);
            setAuthenticated(true);
          });
        } else {
        ...
        if (mounted) { setAuthReady(true); setSessionReady(true); }
      }
    };
    void startAuth();
...
function AuthScreen({ mode, setMode, onAuthenticated }: { mode: 'login' | 'signup' | 'forgot' | 'verify' | 'reset' | 'success'; setMode: (mode: 'login' | 'signup' | 'forgot' | 'verify' | 'reset' | 'success') => void; onAuthenticated: () => void }) {
  ...
  const createAccount=async()=>{ setError(''); const rawUsername=username.trim().replace(/^@+/,'').toLowerCase(); const normalizedUsername=rawUsername?`@${rawUsername}`:''; if(!name.trim()||!rawUsername||!email.trim()||!password)return setError('Please enter your name, username, email and password.'); if(!/^[a-z0-9_]{3,20}$/i.test(rawUsername))return setError('Username must use 3-20 letters, numbers or underscores.'); if(password.length<8)return setError('Password must be at least 8 characters.'); if(password!==confirmPassword)return setError('Passwords do not match.'); if(!remember)return setError('Please agree to the Terms & Privacy Policy.'); try{ const cred=await createUserWithEmailAndPassword(auth,email.trim(),password); await updateProfile(cred.user,{displayName:name.trim()}); await api.post('/api/auth/register',{name:name.trim(),username:normalizedUsername}); onAuthenticated(); }catch(e:any){ const code=String(e?.code||''); setError(code==='auth/email-already-in-use'?'This email is already registered. Please Login.':String(e?.message||'Could not create account.').replace(/^Firebase:\s*/i,'')); } };
...
function GoogleOnboarding({onComplete}:{onComplete:()=>void}){
 ...
 const submit=async()=>{setError(''); const rawUsername=username.trim().replace(/^@+/,'').toLowerCase(); const normalizedUsername=rawUsername?`@${rawUsername}`:''; if(!name.trim()||!rawUsername||!password)return setError('Name, username and password are required.'); if(!/^[a-z0-9_]{3,20}$/i.test(rawUsername))return setError('Username must use 3-20 letters, numbers or underscores.'); if(password.length<8)return setError('Password must be at least 8 characters.'); if(password!==confirm)return setError('Passwords do not match.'); setSaving(true); try{await updatePassword(user,password);await updateProfile(user,{displayName:name.trim()});await api.post('/api/auth/register',{name:name.trim(),username:normalizedUsername,avatar:user.photoURL||null});onComplete();}catch(e:any){setError(String(e?.message||'Could not complete registration.').replace('Firebase: ',''));}finally{setSaving(false)}};
 return <main className='auth-screen'><div className='auth-brand'><PardaisLiteLogo compact/><h1>COMPLETE REGISTRATION</h1><span>CONTINUE WITH GOOGLE</span></div><div className='auth-card'><p>Your Google account is connected. Create your Pardais Lite password and username once. Your username cannot be changed later.</p><AuthField label='NAME' value={name} type='text' placeholder='Your name' icon={<UserRound/>} onChange={setName}/><label className='auth-field'><span>USERNAME</span><div><span className='auth-field-icon'><UserRound/></span><span className='username-prefix'>@</span><input value={username.replace(/^@+/,'')} type='text' placeholder='username' maxLength={20} onChange={e=>setUsername(e.target.value.replace(/[^a-zA-Z0-9_]/g,'').slice(0,20))}/></div></label><AuthField label='PASSWORD' value={password} type='password' placeholder='Create password' icon={<LockKeyhole/>} onChange={setPassword}/><AuthField label='CONFIRM PASSWORD' value={confirm} type='password' placeholder='Confirm password' icon={<LockKeyhole/>} onChange={setConfirm}/><AuthError message={error}/><button className='auth-primary' disabled={saving} onClick={()=>void submit()}>{saving?'Saving...':'Continue to Pardais'}</button></div></main>;
}
...
function HomeScreen({ homeMode, setHomeMode, onSearch, onOpenProfile }: any) {
  ...
  useEffect(()=>{
    setVideoLoading(Boolean(items[index]?.mediaUrl));
  ...
  const download=async()=>{if(!reel)return;try{const r=await api.post('/api/media/download',{key:reel.key});const a=document.createElement('a');a.href=r.data.url;a.download=`pardais-${reel.id}.mp4`;a.target='_blank';document.body.appendChild(a);a.click();a.remove();setMenuOpen(false)}catch{window.alert('Download unavailable right now.')}};
  const follow=async()=>{if(!reel||followBusy)return;setFollowBusy(true);try{await api.post('/api/follow',{followingId:reel.userId,action:'follow'});setItems(v=>v.map(x=>x.id===reel.id?{...x,viewerFollowing:true}:x))}catch{}finally{setFollowBusy(false)}};
  const sendGift=async(gift:any)=>{if(!reel||giftBusy)return;setGiftBusy(true);try{await api.post('/api/gifts/send',{receiverId:reel.userId,giftId:gift.id,quantity:1,coins:gift.coins});setGiftOpen(false);window.alert(`${gift.name} sent successfully.`)}catch(e:any){window.alert(String(e?.message||'Gift could not be sent.'))}finally{setGiftBusy(false)}};
  const deleteReel=async()=>{if(!reel||reel.userId!==auth.currentUser?.uid)return;try{await api.delete(`/api/reels/${reel.id}`);setItems(v=>v.filter(x=>x.id!==reel.id));setMenuOpen(false)}catch{window.alert('Could not delete video.')}};
  ...
  return <main className='reel-screen home-feed-screen'>
    ...
    {!items.length?<div className='following-empty'><Video/><b>No videos yet</b><span>Real published reels will appear here.</span></div>:<div ref={feedRef} className='reel-feed-scroll'>{items.map((r,i)=><section className='reel-stage home-reel-stage' key={r.id} onDoubleClick={()=>void like()}>
      ...
      <div className='reel-actions reference-reel-actions'>
        ...
        <button onClick={()=>setGiftOpen(true)} aria-label='Send gift'><Gift/><span>Gift</span></button>
        <button onClick={()=>void save()} aria-label='Save video'><Bookmark fill={r.savedByMe?'currentColor':'none'}/><span>Save</span></button>
        <button onClick={()=>{try{void navigator.share?.({title:r.author?.name||'Pardais Reel',text:r.caption||'',url:window.location.href})}catch{}}} aria-label='Share video'><Share2/><span>Share</span></button>
    ...
    {giftOpen&&reel&&<div className='gift-sheet-backdrop' onClick={e=>{if(e.target===e.currentTarget)setGiftOpen(false)}}><div className='gift-sheet'><div className='modal-head'><h2>Send Gift</h2><button onClick={()=>setGiftOpen(false)}><X/></button></div><div className='gift-grid'>{[{id:'rose',name:'Rose',icon:'🌹',coins:10},{id:'heart',name:'Heart',icon:'💖',coins:50},{id:'crown',name:'Crown',icon:'👑',coins:100},{id:'diamond',name:'Diamond',icon:'💎',coins:500}].map(g=><button key={g.id} disabled={giftBusy} onClick={()=>void sendGift(g)}><span>{g.icon}</span><b>{g.name}</b><small>{g.coins} coins</small></button>)}</div></div></div>}
...
function CreateScreen({ mode='upload', onClose, onGoLive }: any) {
 ...
 const [step,setStep]=useState<1|2|3>(1); const [file,setFile]=useState<File|null>(null); const [startingLive,setStartingLive]=useState(false); const [liveCountdown,setLiveCountdown]=useState(''); const [preview,setPreview]=useState(''); const [micOn,setMicOn]=useState(true); const [camOn,setCamOn]=useState(true); const [facing,setFacing]=useState<'user'|'environment'>('user'); const [zoom,setZoom]=useState(1); const [effect,setEffect]=useState(false); const [recording,setRecording]=useState(false); const [uploading,setUploading]=useState(false); const [caption,setCaption]=useState(''); const [location,setLocation]=useState(''); const [hashtags,setHashtags]=useState(''); const [visibility,setVisibility]=useState<'public'|'private'>('public'); const [allowComments,setAllowComments]=useState(true); const [error,setError]=useState(''); const [draftSaved,setDraftSaved]=useState(false); const isLive=mode==='live';
 ...
 const startLiveCountdown = async () => {
  ...
  await onGoLive({camOff:!camOn,muted:!micOn,facing});
};
 if(isLive)return <main className='camera-screen go-live-screen'><div className='camera-preview'><button className='camera-close' onClick={onClose}><X/></button><div className='camera-mode'><span className='ready-dot'/> Ready</div><video ref={videoRef} muted playsInline className='capture-preview' style={filterStyle}/>{!camOn&&<div className='camera-off-preview'><VideoOff/><span>Camera is off</span></div>}<div className='live-reference-controls'><button onClick={()=>{const n=facing==='user'?'environment':'user';setFacing(n);void startCamera(n)}}><span className='control-symbol'>↻</span><small>Flip</small></button><button onClick={()=>setEffect(v=>!v)}><Sparkles/><small>Beauty</small></button><button onClick={()=>{const t=streamRef.current?.getAudioTracks()[0];if(t){t.enabled=!micOn;setMicOn(!micOn)}}}><Mic/><small>{micOn?'Mute':'Unmute'}</small></button><button className={!camOn?'live-cam-off':''} onClick={()=>{const t=streamRef.current?.getVideoTracks()[0];if(t){t.enabled=!camOn;setCamOn(!camOn)}}}>{camOn?<Camera/>:<VideoOff/>}<small>{camOn?'Cam':'Cam Off'}</small></button></div><button className='go-live-reference-button' disabled={startingLive} onClick={()=>void startLiveCountdown()}>{startingLive ? liveCountdown : '● Go Live'}</button>{startingLive&&<div className='go-live-countdown' aria-live='assertive'>{liveCountdown}</div>}</div></main>;
...
function ProfileScreen({ onSettings, onEdit, onFollowers, onShare, onLevel, profileTab, setProfileTab, onCreator, onAgency, onWallet }: any) {
  ...
  const avatar = user?.avatar || auth.currentUser?.photoURL || '';
  return <main className="page profile-page"><div className="profile-header"><h1>Profile</h1><div><button onClick={onEdit}><Edit3 /></button><button onClick={onShare}><Share2 /></button><button><Bell /></button><button onClick={onSettings}><Settings /></button></div></div>
    <div className="profile-identity"><button className="profile-avatar" onClick={()=>avatar&&setAvatarOpen(true)} aria-label="Open profile picture">{avatar ? <img src={avatar} alt="Profile" /> : 'P'}</button><div className="verified">✓</div><h2>{displayName}</h2><div className="handle">@{user?.username || userId.slice(0, 10)} <Copy size={15} /></div><p>{user?.bio || 'No bio yet'}</p><button className="profile-level-chip" onClick={onLevel}><span>Lv</span><b>{user?.level || 1}</b><small>Pardais Member</small><ChevronRight /></button></div>
    ...
    <div className="profile-tabs"><button className={profileTab === 'Public' ? 'selected' : ''} onClick={() => setProfileTab('Public')}><Globe2 /> Public</button><button className={profileTab === 'Private' ? 'selected' : ''} onClick={() => setProfileTab('Private')}>♧ Private</button><button className={profileTab === 'Saved' ? 'selected' : ''} onClick={() => setProfileTab('Saved')}><Bookmark /> Saved</button><button className={profileTab === 'Drafts' ? 'selected' : ''} onClick={() => setProfileTab('Drafts')}>Drafts</button></div><ProfileVideoArea mode={profileTab} />
    {avatarOpen&&<div className="profile-avatar-viewer" onClick={e=>{if(e.target===e.currentTarget)setAvatarOpen(false)}}><button className="profile-avatar-close" onClick={()=>setAvatarOpen(false)}><X/></button><img src={avatar} alt="Profile picture"/><a className="profile-avatar-save" href={avatar} download="pardais-profile.jpg" target="_blank" rel="noreferrer">Save</a></div>}
  </main>;
}
function ProfileVideoArea({ mode }: { mode: 'Public' | 'Private' | 'Saved' | 'Drafts' }) {
  const [videos,setVideos]=useState<any[]>([]); const [selected,setSelected]=useState<any|null>(null); const [menuId,setMenuId]=useState<string|null>(null); const uid=auth.currentUser?.uid||'';
  useEffect(()=>{const load=async()=>{try{if(mode==='Saved'){const r=await api.get('/api/reels?mine=0');setVideos((r.data?.items||[]).filter((x:any)=>x.savedByMe));return}const r=await api.get('/api/reels?mine=1');setVideos((r.data?.items||[]).filter((x:any)=>mode==='Public'?x.status==='published'&&x.visibility!=='private':mode==='Private'?x.status==='published'&&x.visibility==='private':x.status==='draft'))}catch{setVideos([])}};void load()},[mode,uid]);
  const publish=async(id:string)=>{try{await api.patch(`/api/reels/${id}`,{status:'published',visibility:'public'});setVideos(v=>v.filter(x=>x.id!==id));setMenuId(null)}catch{window.alert('Could not publish draft.')}};
  const remove=async(id:string)=>{try{await api.delete(`/api/reels/${id}`);setVideos(v=>v.filter(x=>x.id!==id));setMenuId(null);setSelected(null)}catch{window.alert('Could not delete video.')}};
  return <div className='profile-video-area'>
    {videos.length?<div className='video-grid'>{videos.map(v=><div key={v.id} className='video-tile' onClick={()=>setSelected(v)}>
      <video src={v.mediaUrl} muted playsInline preload='metadata'/>
      <div className='video-stats'><span>▶ {v.viewsCount||v.views||0}</span><span>♥ {v.likesCount||0}</span></div>
      {mode==='Drafts'&&<button className='tile-more' onClick={e=>{e.stopPropagation();setMenuId(menuId===v.id?null:v.id)}}><MoreHorizontal/></button>}
      {mode!=='Saved'&&mode!=='Drafts'&&<button className='tile-more' onClick={e=>{e.stopPropagation();setMenuId(menuId===v.id?null:v.id)}}><MoreHorizontal/></button>}
      {menuId===v.id&&<div className='profile-tile-menu' onClick={e=>e.stopPropagation()}>{mode==='Drafts'&&<button onClick={()=>void publish(v.id)}>Publish</button>}<button className='danger' onClick={()=>void remove(v.id)}>Delete Video</button></div>}
    </div>)}</div>:<div className='empty-state'><Video/><b>No {mode.toLowerCase()} videos</b><span>{mode==='Drafts'?'Saved drafts will appear here.':'Uploaded production videos will appear here.'}</span></div>}
    {selected&&<div className='profile-video-viewer' onClick={e=>{if(e.target===e.currentTarget)setSelected(null)}}><button className='viewer-close' onClick={()=>setSelected(null)}><X/></button><video src={selected.mediaUrl} controls autoPlay playsInline/><div className='viewer-info'><b>{selected.caption||'Pardais Reel'}</b><span>♥ {selected.likesCount||0} · ▶ {selected.viewsCount||selected.views||0}</span></div></div>}
  </div>;
}
...
function UserProfilePage({ user, following, onFollow, onBack }: any) { const targetId=String(user?.id||''); const [data,setData]=useState<any>(null); const [avatarOpen,setAvatarOpen]=useState(false); useEffect(()=>{if(targetId)void api.get(`/api/users/${encodeURIComponent(targetId)}`).then(r=>setData(r.data?.user)).catch(()=>{})},[targetId]); const avatar=data?.avatar||''; return <main className='full-dark-page user-profile-page'><header className='simple-page-head'><button onClick={onBack}><ChevronLeft/></button><h1>Profile</h1><span/></header><section className='user-hero'><button className='user-big-avatar' onClick={()=>avatar&&setAvatarOpen(true)}>{avatar?<img src={avatar} alt='Profile'/>:'P'}</button><h2>{data?.name||'Pardais User'}</h2><p>{data?.username||''}</p><div className='user-actions'><button className={following?'following-btn':'follow-btn'} onClick={onFollow}>{following?'Following':'Follow'}</button></div></section>{avatarOpen&&<div className='profile-avatar-viewer' onClick={e=>{if(e.target===e.currentTarget)setAvatarOpen(false)}}><button className='profile-avatar-close' onClick={()=>setAvatarOpen(false)}><X/></button><img src={avatar} alt='Profile picture'/><a className='profile-avatar-save' href={avatar} download='pardais-profile.jpg' target='_blank' rel='noreferrer'>Save</a></div>}</main>; }
...
function EditProfilePage({ onBack }: any) {
  const userId = auth.currentUser?.uid || '';
  const [form, setForm] = useState({ username: '', firstName: '', lastName: '', gender: '', dateOfBirth: '', bio: '', phone: '', whatsapp: '', facebook: '', instagram: '', youtube: '', avatar: '' });
  const [message, setMessage] = useState(''); const [saving,setSaving]=useState(false); const [uploading,setUploading]=useState(false); const [nameLockedUntil,setNameLockedUntil]=useState(''); const [originalName,setOriginalName]=useState('');
  const email = auth.currentUser?.email || '';
  useEffect(() => { void api.get(`/api/profile/${userId}`).then(r => { const p=r.data?.profile||{}; const displayName=auth.currentUser?.displayName||''; const firstName=p.firstName ?? displayName.split(' ')[0] ?? ''; const lastName=p.lastName ?? displayName.split(' ').slice(1).join(' '); setForm(v => ({ ...v, ...p, username:p.username||'', firstName, lastName })); const loadedName=`${firstName} ${lastName}`.trim(); setOriginalName(loadedName); if(p.nameChangeLockedUntil)setNameLockedUntil(p.nameChangeLockedUntil); else if(p.nameChangedAt){const t=Date.parse(String(p.nameChangedAt)); if(Number.isFinite(t))setNameLockedUntil(new Date(t+7*24*60*60*1000).toISOString());} }).catch(() => {}); }, [userId]);
  ...
  return <SubLayout title="Edit Profile" onBack={onBack} right={<button className='edit-save-button' disabled={saving||uploading} onClick={()=>void save()}>{saving?'Saving…':'Save'}</button>}><div className="edit-profile">
    {message && <div className='transfer-success'><div>✓</div><p>{message}</p></div>}
    <div className="edit-avatar-wrap"><button type='button' className="edit-avatar" onClick={()=>document.getElementById('pardais-profile-image')?.click()}>{avatar?<img src={avatar} alt='Profile'/>:'P'}</button><button type='button' onClick={()=>document.getElementById('pardais-profile-image')?.click()}><Camera /></button><input id='pardais-profile-image' type='file' accept='image/*' hidden onChange={e=>void uploadAvatar(e.target.files?.[0])}/></div><p className="change-picture">{uploading?'Uploading…':'Tap to change profile picture'}</p>
    <div className="name-grid">{editableField('FIRST NAME','firstName',form.firstName,Boolean(nameLockedUntil&&Date.now()<Date.parse(nameLockedUntil)))}{editableField('LAST NAME','lastName',form.lastName,Boolean(nameLockedUntil&&Date.now()<Date.parse(nameLockedUntil)))}</div>
    {nameLockedUntil&&Date.now()<Date.parse(nameLockedUntil)&&<div className='name-lock-note'>Name changes are available again after {new Date(nameLockedUntil).toLocaleDateString()}.</div>}
    {editableField('USERNAME','username',String(form.username||''),true)}
    {editableField('EMAIL','email',email,true)}
    {editableField('PHONE NUMBER','phone',form.phone)}
    {editableField('AGE','age',age,true)}
    {editableField('GENDER','gender',form.gender)}{editableField('DATE OF BIRTH','dateOfBirth',form.dateOfBirth)}
    <label className='field'><span className='field-label'>BIO <span>{form.bio.length}/150</span></span><div className='input-box'><input maxLength={150} value={form.bio} onChange={e => set('bio', e.target.value)} placeholder='Tell us about yourself...' /></div></label>
