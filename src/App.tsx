...
function EditProfilePage({ onBack }: any) {
  ...
  useEffect(() => { void api.get(`/api/profile/${userId}`).then(r => { const p=r.data?.profile||{}; setForm(v => ({ ...v, ...p, username:p.username||'', firstName:p.firstName ?? (auth.currentUser?.displayName||'').split(' ')[0]||'', lastName:p.lastName ?? (auth.currentUser?.displayName||'').split(' ').slice(1).join(' ') })); const loadedName=`${p.firstName ?? (auth.currentUser?.displayName||'').split(' ')[0]||''} ${p.lastName ?? (auth.currentUser?.displayName||'').split(' ').slice(1).join(' ')||''}`.trim(); setOriginalName(loadedName); if(p.nameChangeLockedUntil)setNameLockedUntil(p.nameChangeLockedUntil); else if(p.nameChangedAt){const t=Date.parse(String(p.nameChangedAt)); if(Number.isFinite(t))setNameLockedUntil(new Date(t+7*24*60*60*1000).toISOString());} }).catch(() => {}); }, [userId]);
