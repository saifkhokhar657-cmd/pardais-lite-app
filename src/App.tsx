...
function HomeScreen({ homeMode, setHomeMode, liked, setLiked, saved, setSaved, followed, setFollowed, onSearch }: any) {
 const [items,setItems]=useState<any[]>([]); const [index,setIndex]=useState(0); const [comments,setComments]=useState<any[]>([]); const [commentText,setCommentText]=useState(''); const [commentOpen,setCommentOpen]=useState(false); const [uploading,setUploading]=useState(false); const fileRef=useRef<HTMLInputElement|null>(null);
 const load=async()=>{try{const r=await api.get('/api/feed');setItems(r.data?.items||[]);setIndex(0);}catch{setItems([])}}; useEffect(()=>{void load()},[]);
 const reel=items[index]||null;
 const like=async()=>{if(!reel)return;try{const r=await api.post(`/api/reels/${reel.id}/like`);setItems(v=>v.map((x,i)=>i===index?{...x,likesCount:r.data.likesCount,likedByMe:r.data.liked}:x));setLiked(Boolean(r.data.liked));}catch{}};
 ...
 const sendComment=async()=>{const text=commentText.trim();if(!text||!reel)return;try{await api.post('/api/comments',{targetId:reel.id,text});setCommentText('');await loadComments();}catch{}}
 const upload=async(file:File)=>{if(!file.type.startsWith('video/'))return;setUploading(true);try{const p=await api.post('/api/media/presign',{fileName:file.name,contentType:file.type});const put=await fetch(p.data.url,{method:'PUT',headers:{'Content-Type':file.type},body:file});if(!put.ok)throw new Error('R2 upload failed');await api.post('/api/reels',{key:p.data.key,mediaUrl:p.data.publicUrl,caption:file.name.replace(/\.[^.]+$/,'')});await load();}catch(e:any){window.alert(e?.message||'Upload failed.')}finally{setUploading(false)}};
 return <main className='reel-screen home-feed-screen'><div className='reel-top home-feed-top'><button className={homeMode==='Following'?'tab active':'tab muted'} onClick={()=>{setHomeMode('Following');setIndex(0)}}>Following</button><button className={homeMode==='For You'?'tab active':'tab muted'} onClick={()=>{setHomeMode('For You');setIndex(0)}}>For You</button><button className='search-icon' onClick={onSearch} aria-label='Search'><Search/></button><button className='search-icon' onClick={()=>fileRef.current?.click()} aria-label='Upload reel'><Plus/></button><input ref={fileRef} type='file' accept='video/*' hidden onChange={e=>{const f=e.target.files?.[0];if(f)void upload(f);e.currentTarget.value=''}}/></div>
 {!reel?<div className='following-empty'><Video/><b>{uploading?'Uploading…':'No videos yet'}</b><span>Real uploaded reels will appear here. Use + to upload the first video.</span></div>:<div className='reel-stage home-reel-stage' onDoubleClick={()=>void like()}><video src={reel.mediaUrl} controls={false} autoPlay muted loop playsInline style={{width:'100%',height:'100%',objectFit:'cover'}}/><div className='reel-overlay'/><div className='reel-copy'><b>{reel.author?.name||'Pardais User'}</b><small>{reel.author?.username||''}</small><p>{reel.caption||''}</p></div><div className='reel-actions'><button onClick={()=>void like()}><Heart fill={reel.likedByMe?'currentColor':'none'}/><span>{reel.likesCount||0}</span></button><button onClick={()=>{setCommentOpen(true);void loadComments()}}><MessageCircle/><span>Comment</span></button><button onClick={()=>navigator.share?.({url:window.location.href,title:'Pardais Lite Reel'})}><Share2/><span>Share</span></button><button onClick={()=>{if(index<items.length-1)setIndex(index+1)}}><ChevronRight/><span>Next</span></button></div></div>}
 {commentOpen&&<div className='center-modal-backdrop' onClick={e=>{if(e.target===e.currentTarget)setCommentOpen(false)}}><div className='auth-card'><h2>Comments</h2><div style={{maxHeight:220,overflow:'auto'}}>{comments.map((c:any)=><p key={c.id}><b>{c.userId===auth.currentUser?.uid?'You':c.userId}</b> {c.text}</p>)}</div><input value={commentText} onChange={e=>setCommentText(e.target.value)} placeholder='Write a comment'/><button className='auth-primary' onClick={()=>void sendComment()}>Send</button></div></div>}
 </main>;
}
